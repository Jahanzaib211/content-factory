                ...
                try:
                    ...
                    for i, clip in enumerate(clips):
                        ...
                        if os.path.exists(clip_path):
                            gallery.add(
                                title=clip.get("text", f"Clip {i+1}"),
                                caption=clip.get("text", "")[:500],
                                source="clip",
                                template_id="clip_generator",
                                file_path=clip_path,
                                file_type="video",
                                file_size=os.path.getsize(clip_path),
                                metadata={"job_id": job_id, "start": clip.get("start"), "end": clip.get("end")},
                                tags=clip.get("hashtags", []),
                            )
                except Exception as gal_err:
                    log.warning(f"Gallery save failed for clip job {job_id}: {gal_err}")
...
def _factory_run_job(job_id: str) -> None:
    ...
    Runs in a thread via run_in_executor. Saves outputs to gallery on completion."""
    ...
    try:
        ...
        # Save completed outputs to gallery
        try:
            from engines.gallery import GalleryStore
            gallery = GalleryStore()
            for out in job["outputs"]:
                ...
                if out_path and os.path.exists(out_path):
                    ...
                    file_type = "video" if ext in (".mp4", ".mov", ".webm") else "image" if ext in (".png", ".jpg", ".jpeg") else "audio" if ext in (".mp3", ".wav", ".aac") else "text"
                    gallery.add(
                        title=out.get("name", job.get("name", job["template_id"])),
                        caption=f"Generated by {job['template_id']} template",
                        source="factory",
                        template_id=job["template_id"],
                        file_path=out_path,
                        file_type=file_type,
                        file_size=out.get("size", 0),
                        metadata={"job_id": job_id, "rank": out.get("rank"), "cost": job.get("cost_estimate", {})},
                        tags=[job["template_id"]],
                    )
        except Exception as gal_err:
            log.warning(f"Gallery save failed for factory job {job_id}: {gal_err}")
    except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════
# GALLERY ENDPOINTS — unified content store for all pipeline outputs
# ═══════════════════════════════════════════════════════════════════════

@app.get("/api/gallery")
async def gallery_list(
    source: str = "", template_id: str = "", status: str = "",
    tag: str = "", search: str = "", limit: int = 100, offset: int = 0,
):
    """List gallery items with optional filters."""
    try:
        from engines.gallery import GalleryStore
        store = GalleryStore()
        items = store.list_items(source=source, template_id=template_id, status=status,
...
async def gallery_stats():
    ...
    try:
        ...
        return {"total_items": 0, "total_size": 0, "by_source": {}, "by_status": {}, "error": str(e)}


@app.get("/api/gallery/{item_id}")
async def gallery_get(item_id: str):
...
async def gallery_update(item_id: str, body: Dict[str, Any] = {}):
    """Update gallery item fields (title, caption, tags, status)."""
    from engines.gallery import GalleryStore
    store = GalleryStore()
    item = store.update(item_id, **body)
    if not item:
        raise HTTPException(404, "Gallery item not found")
    from dataclasses import asdict
    return asdict(item)
...
@app.post("/api/gallery/{item_id}/publish")
async def gallery_publish(item_id: str, body: Dict[str, Any] = {}):
    """Mark gallery item as published to a platform."""
    platform = body.get("platform", "")
    if not platform:
        raise HTTPException(400, "platform required")
    from engines.gallery import GalleryStore
    store = GalleryStore()
    item = store.mark_published(item_id, platform)
    if not item:
        raise HTTPException(404, "Gallery item not found")
    from dataclasses import asdict
    return asdict(item)
...
@app.get("/api/editor/tools")
async def editor_tools():
    """List available video editing operations."""
    return {"tools": [
        {"name": "trim", "description": "Trim video segment", "params": ["start", "duration", "end"]},
        {"name": "merge", "description": "Merge multiple clips", "params": ["clips", "transitions"]},
        {"name": "add_text", "description": "Overlay text on video", "params": ["text", "position", "size", "color"]},
        {"name": "add_audio", "description": "Add/mix audio track", "params": ["audio", "volume", "mix"]},
        {"name": "resize", "description": "Resize or change aspect ratio", "params": ["width", "height", "aspect_ratio"]},
        {"name": "crop", "description": "Crop video region", "params": ["width", "height", "x", "y"]},
        {"name": "rotate", "description": "Rotate or flip video", "params": ["angle", "flip_horizontal", "flip_vertical"]},
        {"name": "speed", "description": "Change playback speed", "params": ["factor"]},
        {"name": "fade", "description": "Fade in/out effects", "params": ["fade_in", "fade_out"]},
        {"name": "filter", "description": "Apply video filter", "params": ["filter_type", "params"]},
        {"name": "chroma_key", "description": "Remove green screen", "params": ["color", "similarity"]},
        {"name": "overlay", "description": "Picture-in-picture", "params": ["overlay", "position", "opacity"]},
        {"name": "subtitles", "description": "Burn subtitles into video", "params": ["subtitle_file"]},
        {"name": "watermark", "description": "Add image watermark", "params": ["image", "position", "opacity"]},
        {"name": "normalize_audio", "description": "Normalize audio loudness", "params": ["target_lufs"]},
        {"name": "extract_audio", "description": "Extract audio track", "params": ["format"]},
        {"name": "thumbnail", "description": "Extract thumbnail frame", "params": ["timestamp"]},
        {"name": "detect_scenes", "description": "Detect scene changes", "params": ["threshold"]},
        {"name": "convert", "description": "Convert video format", "params": ["format", "quality"]},
        {"name": "stabilize", "description": "Stabilize shaky footage", "params": ["smoothing"]},
        {"name": "vignette", "description": "Vignette effect", "params": ["intensity"]},
        {"name": "chromatic_aberration", "description": "Glitch RGB effect", "params": ["intensity"]},
        {"name": "glow", "description": "Bloom/glow effect", "params": ["intensity"]},
        {"name": "text_animated", "description": "Animated text overlay", "params": ["animation", "duration"]},
        {"name": "layout_grid", "description": "Grid layout from clips", "params": ["layout"]},
        {"name": "layout_pip", "description": "Picture-in-picture layout", "params": ["position", "size"]},
        {"name": "repurpose", "description": "Multi-platform export", "params": ["platforms"]},
        {"name": "pipeline", "description": "Chain multiple operations", "params": ["steps"]},
    ]}


@app.get("/api/editor/info")
async def editor_info(video_path: str = ""):
    """Get video metadata."""
    if not video_path or not os.path.exists(video_path):
        raise HTTPException(400, "video_path required and must exist")
    try:
        from engines import video_editor
        return video_editor.info(video_path)
    except Exception as e:
        raise HTTPException(500, f"Failed to get video info: {e}")


@app.post("/api/editor/trim")
async def editor_trim(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"trim_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.trim(video_path, start=body.get("start", "0"),
                                 duration=body.get("duration"), end=body.get("end"),
                                 output=output)
    except Exception as e:
...
async def editor_merge(body: Dict[str, Any] = {}):
    """Merge multiple clips."""
    clips = body.get("clips", [])
    if len(clips) < 2:
        raise HTTPException(400, "At least 2 clips required")
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"merge_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.merge(clips, output=output, transitions=body.get("transitions"))
    except Exception as e:
        raise HTTPException(500, f"Merge failed: {e}")


@app.post("/api/editor/text")
async def editor_text(body: Dict[str, Any] = {}):
    """Overlay text on video."""
    video_path = body.get("video_path", "")
    text = body.get("text", "")
    if not video_path or not text:
        raise HTTPException(400, "video_path and text required")
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"text_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.add_text(video_path, text, position=body.get("position", "top-center"),
                                     size=body.get("size", 48), color=body.get("color", "white"),
                                     start_time=body.get("start_time"), duration=body.get("duration"),
                                     output=output)
    except Exception as e:
        raise HTTPException(500, f"Text overlay failed: {e}")


@app.post("/api/editor/audio")
async def editor_audio(body: Dict[str, Any] = {}):
    """Add or mix audio onto video."""
    video_path = body.get("video_path", "")
    audio_path = body.get("audio_path", "")
    if not video_path or not audio_path:
        raise HTTPException(400, "video_path and audio_path required")
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"audio_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.add_audio(video_path, audio_path, volume=body.get("volume", 1.0),
                                      mix=body.get("mix", False), output=output)
    except Exception as e:
        raise HTTPException(500, f"Audio mix failed: {e}")


@app.post("/api/editor/resize")
async def editor_resize(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"resize_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.resize(video_path, width=body.get("width"), height=body.get("height"),
                                   aspect_ratio=body.get("aspect_ratio"), output=output)
    except Exception as e:
        raise HTTPException(500, f"Resize failed: {e}")


@app.post("/api/editor/crop")
async def editor_crop(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"crop_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.crop(video_path, width=body.get("width"), height=body.get("height"),
                                 x=body.get("x"), y=body.get("y"), output=output)
    except Exception as e:
        raise HTTPException(500, f"Crop failed: {e}")


@app.post("/api/editor/filter")
async def editor_filter(body: Dict[str, Any] = {}):
    """Apply video filter."""
    video_path = body.get("video_path", "")
    filter_type = body.get("filter_type", "")
    if not video_path or not filter_type:
        raise HTTPException(400, "video_path and filter_type required")
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"filter_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.filter(video_path, filter_type=filter_type,
                                   params=body.get("params"), output=output)
    except Exception as e:
        raise HTTPException(500, f"Filter failed: {e}")


@app.post("/api/editor/normalize-audio")
async def editor_normalize_audio(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"norm_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.normalize_audio(video_path, target_lufs=body.get("target_lufs", -16.0),
                                            output=output)
    except Exception as e:
        raise HTTPException(500, f"Normalize failed: {e}")


@app.post("/api/editor/detect-scenes")
async def editor_detect_scenes(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        return video_editor.detect_scenes(video_path, threshold=body.get("threshold", 0.3))
    except Exception as e:
        raise HTTPException(500, f"Scene detection failed: {e}")


@app.post("/api/editor/thumbnail")
async def editor_thumbnail(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output = os.path.join("output", "edited", f"thumb_{uuid.uuid4().hex[:8]}.jpg")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.thumbnail(video_path, timestamp=body.get("timestamp"), output=output)
    except Exception as e:
        raise HTTPException(500, f"Thumbnail failed: {e}")


@app.post("/api/editor/pipeline")
async def editor_pipeline(body: Dict[str, Any] = {}):
    """Run a chain of editing operations."""
    steps = body.get("steps", [])
    if not steps:
        raise HTTPException(400, "steps array required")
    try:
        from engines import video_editor
        import uuid
        output = body.get("output") or os.path.join("output", "edited", f"pipe_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.pipeline(steps, output=output)
    except Exception as e:
        raise HTTPException(500, f"Pipeline failed: {e}")


@app.post("/api/editor/repurpose")
async def editor_repurpose(body: Dict[str, Any] = {}):
    ...
    try:
        from engines import video_editor
        import uuid
        output_dir = os.path.join("output", "repurposed", uuid.uuid4().hex[:8])
        os.makedirs(output_dir, exist_ok=True)
        return video_editor.repurpose(video_path, output_dir=output_dir,
                                      platforms=body.get("platforms"))
    except Exception as e:
        raise HTTPException(500, f"Repurpose failed: {e}")


@app.post("/api/editor/convert")
async def editor_convert(body: Dict[str, Any] = {}):
    ...
    try:
        ...
        output = os.path.join("output", "edited", f"convert_{uuid.uuid4().hex[:8]}.{fmt}")
        os.makedirs(os.path.dirname(output), exist_ok=True)
        return video_editor.convert(video_path, format=fmt, quality=body.get("quality", "high"),
                                    output=output)
    except Exception as e:
        raise HTTPException(500, f"Convert failed: {e}")


@app.post("/api/editor/quality-check")
async def editor_quality_check(body: Dict[str, Any] = {}):
    """Check video quality."""
    video_path = body.get("video_path", "")
    if not video_path:
        raise HTTPException(400, "video_path required")
    try:
        from engines import video_editor
        return video_editor.quality_check(video_path)
    except Exception as e:
...
async def research_keywords(q: str = ""):
...
async def research_score(topic: str = ""):
...
# ═══════════════════════════════════════════════════════════════════════
# ANALYTICS ENDPOINTS — performance tracking across platforms
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# SCHEDULER ENDPOINTS — smart scheduling, recurring, platform cropping
# ═══════════════════════════════════════════════════════════════════════
