...
Add a new language:
  1. Create src/i18n/<lang>.js exporting a default object of message key->string
  2. Add it to SUPPORTED in this file
  3. The Settings → Multilingual card lets the user pick

The same translations are also exposed to the Python backend
(app.py -> /api/i18n/messages) so the FastAPI can localize any
server-rendered pages (gallery, video pages) the same way.
"""
...
def t(key: str, lang: str = DEFAULT_LANG) -> str:
    ...
    msg = MESSAGES.get(lang, MESSAGES[DEFAULT_LANG]).get(key)
    if msg is not None:
        return msg
    en = MESSAGES[DEFAULT_LANG].get(key)
    if en is not None:
        return en
    return key
...
def all_messages() -> Dict[str, Dict[str, str]]:
    """Return all translations (for the /api/i18n/messages endpoint)."""
    return MESSAGES


__all__ = ["SUPPORTED", "DEFAULT_LANG", "MESSAGES", "t", "all_messages"]
